...
function(test_lib_module_names_different)
    ...
    if("${upper_module}" STREQUAL "${upper_lib}")
        message(FATAL_ERROR "The module ${_each_submodule_this_module} can't have the same name as its library. Note that the names are not case sensitive.")
    endif()
    unset(upper_lib)
    unset(upper_module)
    ...
    foreach(_each_submodule_this_module ${THIS_MODULE_SUBMODULE_ORDER})
        ...
        if(EXISTS "${WRAPPER_LIBRARY_SOURCE_DIR}/${_each_submodule_this_module}.wrap")
            ...
            if(EXISTS "${WRAPPER_LIBRARY_SOURCE_DIR}/wrap_${_each_submodule_this_module}.cmake")
                message(FATAL_ERROR "INCORRECT FILE NAME PATTERN: ${WRAPPER_LIBRARY_SOURCE_DIR}/wrap_${_each_submodule_this_module}.cmake should be named ${WRAPPER_LIBRARY_SOURCE_DIR}/${_each_submodule_this_module}.cmake")
            endif()
        ...
        write_changed_cmake_variables_to_file("${WRAPPER_LIBRARY_OUTPUT_DIR}/diff_${_each_submodule_this_module}.diff" "${prelogger_${_each_submodule_this_module}}" "${postlogger_${_each_submodule_this_module}}" "${_each_submodule_this_module}")
